...
- Name: fr05-product-listing.spec.js >> FR-05: Product Listing and Search >> TC09 - Only one h1 tag exists on homepage
- Location: tests\fr05-product-listing.spec.js:135:3
...
```ts
  38  |     await expect(name).toBeVisible();
  39  |     const nameText = await name.textContent();
  40  |     expect(nameText.length).toBeGreaterThan(0);
  41  |     
  42  |     // Verify price exists and contains VND
  43  |     const price = firstProduct.locator('.text-red-500');
