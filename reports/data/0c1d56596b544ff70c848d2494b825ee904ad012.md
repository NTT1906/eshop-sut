...
- Name: fr09-discount-coupons.spec.js >> FR-09: Discount Coupons >> TC03 - Apply expired coupon (EXPIRED)
- Location: tests\fr09-discount-coupons.spec.js:102:3
...
Error: strict mode violation: locator('.text-red-600') resolved to 2 elements:
    ...
    2) <p class="mt-2 text-red-600 text-sm">Mã giảm giá đã hết hạn</p> aka getByText('Mã giảm giá đã hết hạn')
  ...
  - main [ref=e11]:
    - generic [ref=e12]:
      ...
      - generic [ref=e19]:
        ...
        - generic [ref=e21]:
          - textbox "Nhập mã giảm giá..." [ref=e22]: EXPIRED
          - button "Áp dụng" [ref=e23] [cursor=pointer]
        - paragraph [ref=e24]: Mã giảm giá đã hết hạn
...
```ts
  ...
  42  |     
  43  |     const addToCartBtn = page.locator('button:text("Thêm vào giỏ")').first();
  44  |     await addToCartBtn.click();
  45  |     await page.waitForTimeout(1000);
  46  |     
  47  |     // Go to checkout
  48  |     await page.goto(`${BASE_URL}/checkout`);
  49  |     await page.waitForLoadState('networkidle');
  50  |     
  51  |     // Verify coupon input exists
  52  |     const couponInput = page.locator('input[placeholder="Nhập mã giảm giá..."]');
  53  |     await expect(couponInput).toBeVisible();
  54  |     
  55  |     // Verify apply button exists
  56  |     const applyButton = page.locator('button:text("Áp dụng")');
  57  |     await expect(applyButton).toBeVisible();
  58  |   });
  59  | 
  60  |   // TC02: Apply valid percent coupon (SAVE10)
  61  |   test('TC02 - Apply valid percent coupon (SAVE10)', async ({ page }) => {
  62  |     // Login
  63  |     await page.goto(`${BASE_URL}/login`);
  64  |     const inputs = page.locator('form input[type="text"]');
  65  |     await inputs.nth(0).fill('test@eshop.com');
  66  |     await inputs.nth(1).fill('Test1234!');
  67  |     await page.click('button:text("Sign In")');
  ...
  73  |     
  74  |     const addToCartBtn = page.locator('button:text("Thêm vào giỏ")').first();
  75  |     await addToCartBtn.click();
  76  |     await page.waitForTimeout(1000);
  77  |     
  78  |     // Go to checkout
  79  |     await page.goto(`${BASE_URL}/checkout`);
